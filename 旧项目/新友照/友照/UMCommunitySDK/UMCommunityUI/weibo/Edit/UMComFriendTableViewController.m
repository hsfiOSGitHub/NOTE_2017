...
- (void)viewDidLoad {
    ...
    self.dataController =  [UMComUserFollowingDataController userFollowingDataControllerWithUser:[UMComSession sharedInstance].loginUser count:UMCom_Limit_Page_Count];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    UMComUser *user = [self.dataController.dataArray objectAtIndex:indexPath.row];
...
-(void) relayoutCellWithIndexPath:(NSIndexPath *)indexPath cell:(UMComFriendTableViewCell*)cell;
{
    UMComUser *follower = [self.dataController.dataArray objectAtIndex:indexPath.row];
    NSString *iconUrl = [follower iconUrlStrWithType:UMComIconSmallType];
    ...
    cell.nameLabel.textColor = UMComColorWithHexString(@"#666666");;
    [cell.nameLabel setText:follower.name];
}
