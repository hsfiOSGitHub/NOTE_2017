...
//        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, 320, 216);
    ...
    //赋值
    for (int i=0; i<UUPICKER_HOUR; i++) {
        NSString *num = [NSString stringWithFormat:@"%02d",i];
       
        if (i<UUPICKER_MINUTE) {
         NSString *num1 = [NSString stringWithFormat:@"%02d",i * 5];
         [minuteArray addObject:num1 ];
        }
    ...
    if (self.datePickerStyle == UUDateStyle_HourMinute) {
        for (int i=0; i<indexArray.count; i++) {
            if (i == 1) {
                 NSInteger hang = [indexArray[i] integerValue];
                 [myPickerView selectRow:hang / 5 inComponent:i animated:NO];
            }else {
...
- (NSArray *)getNowDate:(NSDate *)date
{
    ...
    if (date) {
        ...
        if (self.datePickerStyle == UUDateStyle_HourMinute)
        {
            ...
            NSString *dateString = [formatter1 stringFromDate:day];
            NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
            [formatter setDateFormat:@"yyyyMMddHHmm"];
           
            dateShow = [formatter dateFromString:[NSString stringWithFormat:@"%@%@",dateString,_onetime]];
        }else {
...
-(CGFloat)pickerView:(UIPickerView *)pickerView rowHeightForComponent:(NSInteger)component {
    switch (self.datePickerStyle) {
        ...
        case UUDateStyle_YearMonthDay:{
            if (component==0) return 216/6;
            if (component==1) return 216/6;
            if (component==2) return 216/6;
        }
            break;
        ...
        case UUDateStyle_HourMinute:{
            if (component==0) return 216/4;
            if (component==1) return 216/4;
        }
            break;
