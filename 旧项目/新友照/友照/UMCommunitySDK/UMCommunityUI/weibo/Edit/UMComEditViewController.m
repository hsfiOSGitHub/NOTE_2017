...
- (void)viewDidLoad
{
    ...
    if ([UMComSession sharedInstance].draftFeed && !self.forwardFeed) {
        self.editDataController = [UMComSession sharedInstance].draftFeed;
    }
    
    self.view.backgroundColor = [UIColor whiteColor];
    

    [self.locationView.label setText:self.editDataController.locationDescription];
...
-(void) initPrePostingData
{
    if (self.realTextView && self.editDataController.text) {
        //在函数内部已经加入了self.editDataController.text的数据，此处只要传入空字符串即可
        [self editContentAppendKvoString:@""];
    }
    
    if (self.locationView && self.editDataController.locationDescription) {
        self.locationView.label.text = self.editDataController.locationDescription;
    }
    
    if (self.editDataController.images) {
        [self.originImages addObjectsFromArray:self.editDataController.images];
        [self handleOriginImages:self.editDataController.images];
...
- (void)createTextView
{
    ...
    self.realTextView.text = self.editDataController.text;
...
- (NSArray *)getCheckWords
{
    ...
    for (UMComTopic *topic in self.editDataController.topics) {
    ...
    for (UMComUser *user in self.editDataController.atUsers) {
...
-(void) presentTopicNoticeBgView
{
    ...
    self.topicNoticeBgView.hidden = YES;
    //话题为空的时候就提示用户
    if (self.editDataController.topics.count == 0) {
...
//        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(3, 0, self.topicNoticeBgView.frame.size.width, 25)];
    ...
    label.font = textFont;
    label.textColor = UMComColorWithHexString(@"008bea");
...
- (void)showWhenForwordOldFeed
{
    ...
    self.editDataController.text = feedAndUserNamesText;
    [self.realTextView updateEditTextView];
...
- (NSArray *)getFeedCheckWordsFromFeed:(UMComFeed *)feed
{
    ...
    for (NSString *userName in [feed.related_user valueForKeyPath:@"name"]) {
        if (![checkWords containsObject:userName]) {
            [checkWords addObject:[NSString stringWithFormat:UserNameString,userName]];        }
    }
    for (NSString *topicName in [feed.topics valueForKeyPath:@"name"]) {
...
- (void)handleOriginImages:(NSArray *)images{
    ...
    if (!self.imagesBgView) {
        ...
        self.locationView.backgroundColor = UMComColorWithHexString(@"f5f6fa");
        self.locationView.locationbackgroudColor = self.locationView.backgroundColor;
        self.locationView.locationBlock = ^{
            weakSelf.topicNoticeBgView.hidden = YES;
            UMComLocationListController *locationViewController = [[UMComLocationListController alloc] initWithLocationSelectedComplectionBlock:^(UMComLocation *locationModel) {
                if (locationModel) {
//                    weakSelf.editDataController.location = [[CLLocation alloc] initWithLatitude:locationModel.coordinate.latitude longitude:locationModel.coordinate.longitude];
                    weakSelf.editDataController.location = [[CLLocation alloc] initWithLatitude:locationModel.latitude.doubleValue longitude:locationModel.longitude.doubleValue];
                    weakSelf.editDataController.locationDescription = locationModel.name;
                    [weakSelf.locationView relayoutChildControlsWithLocation:weakSelf.editDataController.locationDescription];
                    
                }
                else{
                    weakSelf.editDataController.location = nil;
                    weakSelf.editDataController.locationDescription = nil;
                    [weakSelf.locationView relayoutChildControlsWithLocation:weakSelf.editDataController.locationDescription];
                }
            }];
...
- (void)viewsFrameChange
{
    ...
    if (self.forwardFeed) {
        ...
        [self.locationView relayoutChildControlsWithLocation:self.editDataController.locationDescription];
        self.realTextView.frame = realTextViewFrame;
    }
}
...
-(void)showAtFriend:(id)sender
{
    ...
    UMComFriendTableViewController *friendViewController = [[UMComFriendTableViewController alloc] initWithUserSelectedComplectionBlock:^(UMComUser *user) {
        ...
        if (![sender isKindOfClass:[NSString class]] || ([sender isKindOfClass:[NSString class]] && [sender isEqualToString:@"@"])) {
            atFriendStr = @"@";
        }
...
- (void)editContentAppendKvoString:(NSString *)appendString
{
    ...
    if (self.editDataController.text.length > 0) {
        editString = [[NSMutableString alloc] initWithString:self.editDataController.text];
    }else{
    ...
    self.editDataController.text = editString;
    [self.realTextView setText:self.editDataController.text];
    [self.realTextView updateEditTextView];
    [self.realTextView becomeFirstResponder];
...
- (void)followsAddOneUser:(UMComUser *)user
{
    ...
    if (self.editDataController.atUsers) {
        [follows addObjectsFromArray:self.editDataController.atUsers];
    }
    if ([user isKindOfClass:[UMComUser class]]) {
        BOOL isInclude = NO;
        for (NSString *name in [self.editDataController.atUsers valueForKeyPath:@"name"]) {
    ...
    self.editDataController.atUsers = follows;
}

- (void)topicsAddOneTopic:(UMComTopic *)topic
{
    NSMutableArray *topics = [NSMutableArray array];
    if (self.editDataController.topics) {
        [topics addObjectsFromArray:self.editDataController.topics];
    }
    if ([topic isKindOfClass:[UMComTopic class]]) {
        BOOL isInclude = NO;
        for (NSString *name in [self.editDataController.topics valueForKeyPath:@"name"]) {
    ...
    self.editDataController.topics = topics;
}
...
{
    self.editDataController.text = textView.text;
}
...
- (NSArray *)editTextViewDidUpdate:(UMComEditTextView *)textView matchWords:(NSArray *)matchWords
{
    ...
    NSMutableArray *userList = [NSMutableArray arrayWithArray:self.editDataController.atUsers];
    NSMutableArray *topicList = [NSMutableArray arrayWithArray:self.editDataController.topics];
    ...
    self.editDataController.atUsers = userList;
    self.editDataController.topics = topicList;
    if (self.editDataController.topics.count > 0) {
        self.topicNoticeBgView.hidden = YES;
    }else{
        self.topicNoticeBgView.hidden = NO;
...
-(NSArray*)postingTopicsArray
{
    NSMutableOrderedSet* resultOrderedSet = [[NSMutableOrderedSet alloc] initWithArray:self.editDataController.topics];
    ...
    if (self.forwardFeed) {
        NSMutableOrderedSet* forwardFeedTopicsSet = [[NSMutableOrderedSet alloc] initWithArray:self.forwardFeed.topics];
        [resultOrderedSet unionOrderedSet:forwardFeedTopicsSet];
    }
...
- (void)postContent
{
    ...
    self.editDataController.text = self.realTextView.text;
    ...
    if (self.topic) {
        ...
        NSRange range = [self.editDataController.text rangeOfString:topicName];
        if (range.length > 0) {
    ...
    if (!self.forwardFeedBackground) {
        if (self.topic) {
            ...
            if (range.length > 0) {
                [self topicsAddOneTopic:self.topic];
                //                [self.editDataController.topics addObject:self.topic];
            }
        ...
        [self forwardFeed:self.forwardFeed response:^(id responseObject, NSError *error) {
            [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
            [self dealWhenPostFeedFinish:@[responseObject] error:error];
        }];
    }
...
- (void)postWithFeedCompletion:(void (^)(id responseObject,NSError *error))response
{
    //feed.text是必传参数  (!feed.text && !feed.images)
    if (!self.editDataController.text && !self.editDataController.images) {
        if (response) {
            NSError *error = [NSError errorWithDomain:@"UMComParamError.Error" code:3 userInfo:@{NSLocalizedFailureReasonErrorKey:@"you must set feed text or feed images"}];// or feed images
            response(nil,error);
            return;
        }
    }
    ...
    [self.editDataController createFeedWithCompletion:^(id responseObject, NSError *error) {
                                          ...
                                          if (error) {
                                              //一旦发送失败会保存到草稿箱
                                              [UMComSession sharedInstance].draftFeed = weakSelf.editDataController;
                                          } else {
...
{
    __weak typeof(self) weakSelf = self;
    self.editDataController.images = images;
    if ([self isPermission_bulletin]) {
        self.selectedFeedTypeBlock = ^(NSNumber *type){
            [weakSelf postWithFeedCompletion:response];
        };
...
- (void)forwardFeed:(UMComFeed *)forwardFeed
         response:(void (^)(id responseObject,NSError *error))response
{
    NSMutableArray *atUsers = [NSMutableArray arrayWithCapacity:1];
    if (self.editDataController.atUsers.count > 0) {
        [atUsers addObjectsFromArray:self.editDataController.atUsers];
    }
    for (UMComUser *user in self.forwardFeed.related_user) {
    ...
    self.editDataController.atUsers = atUsers;
    self.editDataController.topics = [self postingTopicsArray];
    
    [self.editDataController forwardFeedWithFeed:forwardFeed completion:^(id responseObject, NSError *error) {
        [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
        if (responseObject && [responseObject isKindOfClass:[UMComFeed class]] && !error) {
            if (response) {
                response(responseObject, error);
