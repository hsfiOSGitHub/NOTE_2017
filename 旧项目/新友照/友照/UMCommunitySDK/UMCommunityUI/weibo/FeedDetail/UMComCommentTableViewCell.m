...
- (void)awakeFromNib
{
    ...
    [self.timeLabel setTextColor:UMComColorWithHexString(FontColorGray)];
    ...
    self.nameLabel.textColor = UMComColorWithHexString(UMCom_Micro_Comment_NameTextCollor);
    self.likeCountLabel.font = UMComFontNotoSansLightWithSafeSize(UMCom_Micro_Comment_LikeFontSize);
    self.timeLabel.textColor = UMComColorWithHexString(UMCom_Micro_Comment_TimeTextCollor);
         ...
         commentStyleView:(UMComMutiText *)mutiText
{
    ...
    if ([comment.liked boolValue]) {
        ...
        self.likeCountLabel.textColor = UMComColorWithHexString(@"#008BEA");
    }else{
        [self.likeButton setImage:UMComImageWithImageName(@"um_micro_like_nomal") forState:UIControlStateNormal];
        self.likeCountLabel.textColor = UMComColorWithHexString(@"#A5A5A5");
    }
    ...
    if (comment.image_urls.count > 0) {
        ...
        [self.commentImageView setImages:comment.image_urls placeholder:UMComImageWithImageName(@"um_forum_post_default") cellPad:3];
