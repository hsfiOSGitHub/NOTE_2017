...
@interface Register2HospitalVC ()<UITableViewDataSource,UITableViewDelegate>
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property (nonatomic,strong) NSArray *source;//数据源
@end

static NSString *idnetifierCell = @"idnetifierCell";
@implementation Register2HospitalVC
#pragma mark -懒加载
-(NSArray *)source{
    if (!_source) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    //获取到对应的字典
    NSDictionary *dict = self.source[indexPath.row];
    [ValueHelper sharedHelper].registerHospital = dict;
    [self dismissViewControllerAnimated:YES completion:nil];
