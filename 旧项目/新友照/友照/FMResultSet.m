...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        YZLog(@"Warning: There seem to be no columns in this set.");
    }
...
- (BOOL)nextWithError:(NSError **)outErr {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        YZLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
        YZLog(@"Database busy");
        if (outErr) {
        ...
        YZLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
        ...
        // wtf?
        YZLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    YZLog(@"Warning: I could not find the column named '%@'.", columnName);
