...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"返回"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStylePlain target:self action:@selector(goToBack)];
    ...
    [self.navigationController.navigationBar setTitleTextAttributes:
     @{NSFontAttributeName:[UIFont systemFontOfSize:20],NSForegroundColorAttributeName:navigationBarColor}];
}
...
- (void)goToBack
{
    ...
    if (self.navigationController.viewControllers.count == 1) {//堆栈里只有一个控制器的时候无法正退出
        //        ZXTabBarController *mainVC = [[ZXTabBarController alloc] init];
        //        [self.sideMenuViewController setContentViewController:mainVC];
