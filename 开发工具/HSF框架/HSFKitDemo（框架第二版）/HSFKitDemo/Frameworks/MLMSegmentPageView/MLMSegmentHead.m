...
@property (nonatomic,strong) NSArray *titleIconsArr;
...
- (void)createBtn:(NSArray *)titlesArr addScroll:(UIScrollView*)scroll startX:(CGFloat)start_x start_index:(NSInteger)start_index {
    ...
    for (NSInteger i = start_index; i < titlesArr.count; i ++) {
        ...
        if ([obj isKindOfClass:[NSDictionary class]]) {
            ...
            if (i == 0) {
                [button setImage:[UIImage imageNamed:dic[@"sel_icon"]] forState:UIControlStateNormal];
            }
        }
        ...
        //>>>>>>>>>>>>>
        if ([obj isKindOfClass:[NSDictionary class]]) {
            [button layoutButtonWithEdgeInsetsStyle:HSFButtonEdgeInsetsStyleTop imageTitleSpace:10];
        }
...
- (void)selectedHeadTitles:(UIButton *)button {
    ...
    [UIView animateWithDuration:animation_time animations:^{
        if (_headStyle != SegmentHeadStyleSlide) {
            id before_obj = self.titleIconsArr[currentIndex];
            id select_obj = self.titleIconsArr[selectIndex];
            if ([before_obj isKindOfClass:[NSDictionary class]]) {
                [before_btn setImage:[UIImage imageNamed:before_obj[@"icon"]] forState:UIControlStateNormal];
            }
            if ([select_obj isKindOfClass:[NSDictionary class]]) {
                [select_btn setImage:[UIImage imageNamed:select_obj[@"sel_icon"]] forState:UIControlStateNormal];
            }
...
- (void)setSelectIndex:(NSInteger)index {
    ...
    id select_obj = self.titleIconsArr[currentIndex];
