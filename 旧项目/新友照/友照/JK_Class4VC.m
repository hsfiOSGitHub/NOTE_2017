...
@property (weak, nonatomic) IBOutlet UIImageView *icon_score;
@property (weak, nonatomic) IBOutlet UIImageView *icon_exam;
//
...
-(void)setUpSubViews{
    ...
    //    self.icon_score.backgroundColor = KRGB(154, 95, 197, 1);
    //    self.icon_exam.backgroundColor = KRGB(252, 140, 29, 1);
    
    self.icon_wrong.layer.masksToBounds = YES;
    self.icon_collect.layer.masksToBounds = YES;
    self.icon_score.layer.masksToBounds = YES;
    self.icon_exam.layer.masksToBounds = YES;
    
    self.icon_wrong.layer.cornerRadius = 25;
    self.icon_collect.layer.cornerRadius = 25;
    self.icon_score.layer.cornerRadius = 25;
    self.icon_exam.layer.cornerRadius = 25;
}

//顺序练习
- (IBAction)practice_orderBtnACTION4:(UIButton *)sender {
...
//随机练习
- (IBAction)practice_randomBtnACTION4:(UIButton *)sender {
    QuestionVC *question_VC = [[QuestionVC alloc]init];
    //取出科一所有的题
    question_VC.classType = @"class4";
    NSMutableArray *arr = [[ZXTopicManager sharedTopicManager] readAllSubject4Topics];
...
//专项练习
- (IBAction)practice_specialBtnACTION4:(UIButton *)sender {
    SpecialQuestionVC *specialQuestion_VC = [[SpecialQuestionVC alloc]init];
    specialQuestion_VC.source = @[@"时间", @"距离", @"速度", @"标线", @"标志", @"手势", @"信号灯", @"酒驾", @"灯光", @"装置", @"路况"];
    specialQuestion_VC.classType = @"class4";
    [self.navigationController pushViewController:specialQuestion_VC animated:YES];
}
//易错题
- (IBAction)practice_wrongBtnACTION4:(UIButton *)sender {
    QuestionVC *question_VC = [[QuestionVC alloc]init];
    //取出科一所有的题
    question_VC.classType = @"class4";
    NSMutableArray *arr = [[ZXTopicManager sharedTopicManager] readSubjectYiCuoTiWithSubject:@"4"];
    //转模型
    NSMutableArray *modelArr = [NSMutableArray array];
    for (NSDictionary *dic in arr) {
        ZXBaseTopicModel *model = [ZXBaseTopicModel modelWithDic:dic];
...
//未做题
- (IBAction)prectice_unfinishBtnACTION4:(UIButton *)sender {
    QuestionVC *question_VC = [[QuestionVC alloc]init];
    //取出科一所有的题
    question_VC.classType = @"class4";
    NSMutableArray *arr = [[ZXTopicManager sharedTopicManager] readAllWeiZuoTiWithSubject:@"4"];
    //转模型
    NSMutableArray *modelArr = [NSMutableArray array];
    for (NSDictionary *dic in arr) {
        ZXBaseTopicModel *model = [ZXBaseTopicModel modelWithDic:dic];
...
//模拟考试
- (IBAction)exam_mockBtnACTION4:(UIButton *)sender
{
...
//考试须知
- (IBAction)exam_noticeBtnACTION4:(UIButton *)sender
{
    ZX_Web_ViewController *webVC = [[ZX_Web_ViewController alloc] init];
    webVC.htmlStr = @"科目四考试须知";
    webVC.titleStr = @"考试须知";
    [self.navigationController pushViewController:webVC animated:YES];
}

//考前愿望
- (IBAction)exam_wishBtnACtION4:(UIButton *)sender
{
...
- (IBAction)myWrongQuestion:(UITapGestureRecognizer *)sender
{
    ...
    //取出科一所有的题
    question_VC.classType = @"class4";
    NSMutableArray *arr = [[ZXTopicManager sharedTopicManager] readClass4AllWrongTopics];
    if (arr.count == 0)
    {
...
- (IBAction)myCollection:(UITapGestureRecognizer *)sender
{
    ...
    //取出科一所有的题
    question_VC.classType = @"class4";
    NSMutableArray *arr = [[ZXTopicManager sharedTopicManager] readClass4AllFavTopics];
    if (arr.count == 0)
    {
...
//评分标准
- (IBAction)scoring:(UITapGestureRecognizer *)sender
{
    ZX_Web_ViewController *webVC = [[ZX_Web_ViewController alloc] init];
    webVC.htmlStr = @"科四评分标准";
    webVC.titleStr = @"科四评分标准";
    [self.navigationController pushViewController:webVC animated:YES];
}

//考试秘笈
- (IBAction)examCheats:(UITapGestureRecognizer *)sender
{
    ZX_Web_ViewController *webVC = [[ZX_Web_ViewController alloc] init];
    webVC.htmlStr = @"科四考试秘籍";
    webVC.titleStr = @"科四考试秘籍";
    [self.navigationController pushViewController:webVC animated:YES];
}
