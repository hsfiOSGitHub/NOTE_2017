...
/** 自动页面时长统计, 开始记录某个页面展示时长.
 使用方法：必须配对调用beginLogPageView:和enYZLogPageView:两个函数来完成自动统计，若只调用某一个函数不会生成有效数据。
 在该页面展示时调用beginLogPageView:，当退出该页面时调用enYZLogPageView:
...
/** 自动页面时长统计, 结束记录某个页面展示时长.
 ...
 @return void.
 */
+ (void)enYZLogPageView:(NSString *)pageName;
