...
- (void)resetWithFeed:(UMComFeed *)feed
{
    ...
    if (feed.text && [feed.status intValue] < FeedStatusDeleted) {
        NSInteger feedTextLength = [UMComKit getStringLengthWithString:self.feed.text];
        ...
        UMComMutiText *mutitext = [UMComMutiText mutiTextWithSize:CGSizeMake(self.subViewWidth, MAXFLOAT) font:FeedFont string:feedSting lineSpace:UMCom_Micro_FeedContent_LineSpace checkWords:feedCheckWords textColor:UMComColorWithHexString(UMCom_Micro_Feed_ContentCollor)];
        ...
        if (feedTextLength > UMCom_Micro_FeedContentLength && self.isDisplayAllContent) {
            mutitext.textSize = CGSizeMake(mutitext.textSize.width, mutitext.textSize.height);
        }
    }
    ...
    if (origin_feed) {
        ...
        if ([UMComKit getStringLengthWithString:oringFeedString] > UMCom_Micro_FeedContentLength && self.isDisplayAllContent == NO) {
        ...
        if (origin_feed.location) {
            ...
            self.locationModel = origin_feed.location;
        }
        self.imageModels = origin_feed.image_urls;
        ...
        if (feed.location) {
            ...
            self.locationModel = feed.location;
        }
        self.imageGridViewOriginX = 0;
        self.imageModels = feed.image_urls;
        if (self.imageModels.count > 0) {
