...
//自己补充的方法
- (UIColor *)preferredNormalColorForDate:(NSDate *)date;
...
- (void)invalidateAppearanceForCell:(FSCalendarCell *)cell
{
    ...
    cell.preferredNormalColor = [self preferredNormalColorForDate:cell.date];
...
//响应我的代理方法
- (UIColor *)preferredNormalColorForDate:(NSDate *)date {
    if (self.delegateAppearance && [self.delegateAppearance respondsToSelector:@selector(calendar:appearance:normalColorForDate:)] ) {
        UIColor *color = [self.delegateAppearance calendar:self appearance:self.appearance normalColorForDate:date];
        return color;
    }
    return nil;
}
