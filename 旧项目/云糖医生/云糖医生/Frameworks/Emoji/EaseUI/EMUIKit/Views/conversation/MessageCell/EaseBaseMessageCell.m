...
@property(nonatomic)BOOL hehe;
@property(nonatomic,strong)NSMutableDictionary* dict;
@property(nonatomic)NSInteger ge;
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    if (!self.model.isSender)
    {
        if (!_dict)
        {
            ...
            {
                if ([_liaotian[i][@"phone"] isEqualToString:model.message.conversationId])
                {
                    _dict=[NSMutableDictionary dictionaryWithDictionary:_liaotian[i]];
                    _ge=i;
                    continue;
                }
            }
        }
        if ([_dict[@"pic"] isKindOfClass:[NSString class]] && [_dict[@"pic"] length]>0)
        {
            [self.avatarView sd_setImageWithURL:[NSURL URLWithString:_dict[@"pic"]] placeholderImage:[UIImage imageNamed:@"默认患者头像"]];
        }
        else
        {
            self.avatarView.image=[UIImage imageNamed:@"默认患者头像"];
        }
        if ([_dict[@"name"] isKindOfClass:[NSString class]] && [_dict[@"name"] length]>0)
        {
    ...
    {
        if ([[ZXUD objectForKey:@"name"] isKindOfClass:[NSString class]] && [[ZXUD objectForKey:@"name"] length]>0)
        {
            _nameLabel.text = [ZXUD objectForKey:@"name"];
        }
        else
        {
            _nameLabel.text=model.nickname;
        ...
        if ([[ZXUD objectForKey:@"pic"] isKindOfClass:[NSString class]] && [[ZXUD objectForKey:@"pic"] length]>0)
        {
            [self.avatarView sd_setImageWithURL:[NSURL URLWithString:[ZXUD objectForKey:@"pic"]] placeholderImage:[UIImage imageNamed:@"默认医生头像"]];
        }
        else
        {
            self.avatarView.image=[UIImage imageNamed:@"默认医生头像"];
        }
